...
export default function DashboardPage() {
  ...
  const handleAddOperator = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!referentId || !selectedMunicipality?.id) return;
              ...
              {municipalities.map((m) => (
                <div key={m.id} className="py-4 flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-sm text-gray-500">Referente: {m.contact_name || "—"}</p>
                  </div>
                  <span className="text-xs text-blue-700 bg-blue-50 px-2 py-1 rounded-full font-medium">Attivo</span>
                </div>
              ))}
            </div>
          ) : (
